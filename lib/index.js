...
function deref(json, options) {
  ...
  return ((typeof json === 'undefined' ? 'undefined' : _typeof(json)) !== 'object' ? getJsonResource(json, {}) : Promise.resolve({ raw: json, parsed: Array.isArray(json) ? [] : {} })).then(function (_ref) {
    var raw = _ref.raw;
  ...
  function getJsonResource(url, params) {
    return new Promise(function (accept, reject) {
      ...
      if (cached) {
        ...
        var defaultLoader = fileLoader;

        return (options.externalLoader ? options.externalLoader(url, params, defaultLoader) : defaultLoader(url)).then(function (data) {
